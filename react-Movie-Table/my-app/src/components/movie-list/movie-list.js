...
const MovieList = ({ data, onDelete }) => {
    ...
    return (
        <ul className="movie-list">
            ...
            {data.map((item) => (
                <MovieListItem
                    key={item.id} //UNIQUE YANI UNIKAL YANI TAKRORLANMAS id berilmasa react har yangi component qo'shilganda hammasini qaytadan quradi yani 2 ta component bor bo'lsayu 3 chisi qo'shilsa shunda react oldin yasalgan 2ta componentniham qaytadan yasaydi agar id bilan qo'yilsa faqat yangi qo'shilgan idgina qo'shiladi va yana component serverdan massiv holatda qaytganda 0 dan boshlanadigan tartib raqamiham o'zgarmaydi yani ichidan birirtasi udalit qilinsa faqat udalit qilingan component udalit boladi va tartib raqam o'zgarmeydi
                    name={item.name}
