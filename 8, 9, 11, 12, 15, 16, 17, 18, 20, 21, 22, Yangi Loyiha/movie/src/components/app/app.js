...
class App extends Component {
    ...
    onDelete = (id) => {
        ...
        this.setState(({ data }) => ({//datani parametrni dumaloq qavusidan ichida jingalaq qavusda chaqirilishi shu funksiyani parametrida data object holatda chaqirilganini ifoda qiladi(distruptatsa)
            ...
            // };
            // const newArr = data.filter((c) => c.id !== id); //yani bu yangi data hissoblanadi yani data serverchani nusxalab massivda qaytaradi filter metodi// !teng emas
            // console.log(newArr); //yani newArr o'zgaruvchida endi data yani servercha bor ondelete chaqirilgan trash classli funksiyaga bosganda ondelete ishlab logda ko'rganda hamma datalar keletganini ko'rish mumkun//endi bu newArr o'zgaruvchi sababli ondelete funksiyasi ishlaganda yani ondelete funksiyasi props qilib MovieList jo'natilgan MovieListda map qilib MovieListItemda props qilib distruptatsa bilan chaqirilgan MovieListItemdagi trash classi bor button bosilganda MovieListItemga chaqirilgan datani name va viewers elementlari udalit bo'ladi//yani qachonki setsateni bilan class constructordagi stateni o'zgartirganda yangi qiymat berish kerak

            data: data.filter((c) => c.id !== id), //yani endi yuoqridagi data servercha newArrga o'zgardi newarrda esa shu data filter qilingan filter qilinganda esa filter yangi massiv qaytaradi yani datani nusxaalab yangi massiv qaytaradi shunday yo'l bilan reactda datalarni yani o'zgaruvchilarni o'zgartirish mumkun//ondelete funksiya ishga tushganda setstate funksiyani ishga tushuradi//returnni ichida yozilishi kerak edi aslida lekin array funkstionda ()<<dan keyin return afatamatik qo'yiladi shu sabab bir qatorga yozildi
